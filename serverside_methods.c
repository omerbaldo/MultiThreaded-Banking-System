...
int main(int argc, char ** argv){
    ...
    n =	bind(socketNumba, (struct sockaddr *) &serverAddressInfo, sizeof(serverAddressInfo));
    if(n<0){
        error("binding error");
    }
    
    //STEP 3: LISTEN FOR INCOMING CONNECITONS ON SOCKET
    listen(sockfd,10); //10 pending connection on queue
				
    //STEP 4: CREATE PRINTING THREAD BEFORE YOU SELECT/ACCEPT INCOMING CONNECTIONS
    pthread_create(&printThread, NULL, server_print, NULL);
    ...
    while(1){ //create select
        ...
        struct sockaddr_storage oppositeEnd; //storage device
        
        oppositeSocket = accept(socketNumba, (struct sockaddr *)&oppositeEnd, &sizeof);
